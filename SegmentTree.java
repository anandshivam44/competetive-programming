...
class SegmentTree {
    ...
    int getSumUtil(int ss, int se, int qs, int qe, int si) {
        ...
        if (qs <= ss && qe >= se) {
            System.out.println(" return " + st[si]);
            return st[si];
        }
    ...
    int trial_2(int ss, int se, int qs, int qe, int si) {
        // If segment of this node is a part of given range, then return
        // the sum of the segment
        if (qs <= ss && qe >= se)
            return st[si];
        ...
        return trial_2(ss, mid, qs, qe, 2 * si + 1) + trial_2(mid + 1, se, qs, qe, 2 * si + 2);
    }
    ...
    int constructSTUtil(int arr[], int ss, int se, int si) {
        ...
        if (ss == se) {
            // System.out.println("inside if "+ss+"="+se);
            st[si] = arr[ss];
            // System.out.println("\n--- st["+si+"] = "+arr[ss]+" ---\n");
            // System.out.println(" st["+si+"]"+" = "+arr[ss]);
            // System.out.println("return "+arr[ss]);
            return arr[ss];
        }
        ...
        int mid = getMid(ss, se);
        // System.out.println("Mid value of "+ss+ " and "+se +" = "+mid);
        st[si] = constructSTUtil(arr, ss, mid, si * 2 + 1) + constructSTUtil(arr, mid + 1, se, si * 2 + 2);
        // System.out.println(" "+trial(arr, ss, mid, si * 2 + 1) + " + " + trial(arr,
        // mid + 1, se, si * 2 + 2)+" = "+(trial(arr, ss, mid, si * 2 + 1) +
        // trial(arr, mid + 1, se, si * 2 + 2)));
        // System.out.println("\n assigning st["+si+"] = "+st[si]+"\n");
        // System.out.println("--return--"+st[si]);
        return st[si];
    }

    int trial(int arr[], int ss, int se, int si) {
        // If there is one element in array, store it in current node of
        // segment tree and return
        if (ss == se) {
            // System.out.println("inside if "+ss+"="+se);
